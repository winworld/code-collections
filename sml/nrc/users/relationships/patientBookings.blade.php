...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-patientBookings">
                <thead>
                    <tr>
                        <th width="10">
                        ...
                        <th>
                            {{ trans('cruds.booking.fields.booking_number') }}
                        </th>
                        <th>
                            {{ trans('cruds.booking.fields.booking_date') }}
                        </th>
                        <th>
                            {{ trans('cruds.booking.fields.patient') }}
                        </th>
                        <th>
                            {{ trans('cruds.booking.fields.professional') }}
                        </th>
                        <th>
                            {{ trans('cruds.booking.fields.medical_test') }}
                        </th>
                        <th>
                            {{ trans('cruds.booking.fields.total_amount') }}
                        </th>
                        <th>
                ...
                <tbody>
                    @foreach($bookings as $key => $booking)
                        <tr data-entry-id="{{ $booking->id }}">
                            ...
                            <td>
                                @foreach($booking->medical_tests as $key => $item)
                            ...
                            <td>
                                {{ App\Models\Booking::PAYMENT_STATUS_SELECT[$booking->payment_status] ?? '' }}
                            </td>
                            <td>
                                {{ App\Models\Booking::BOOKING_STATUS_SELECT[$booking->booking_status] ?? '' }}
                            </td>
                            <td>
                                @can('booking_show')
...
@endcan
  ...
  let table = $('.datatable-patientBookings:not(.ajaxTable)').DataTable({ buttons: dtButtons })
