...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.store") }}" enctype="multipart/form-data">
            ...
            <div
                class="form-group nrc-group"
                style="display:{{ old('role_id') == config('constants.role_id.patient') ? 'block' : 'none'}};"
            ...
            >
                <div class="form-group">
                    ...
                    <div class="form-row">
                        <div class="col-3">
                            <select class="form-control {{ $errors->has('meta.nrc_state') ? 'is-invalid' : '' }}" name="meta[nrc_state]" id="nrc_state">
                                <option value="">--</option>
                            </select>
                        </div>
                        <div class="col-3">
                            <select class="form-control {{ $errors->has('meta.nrc_township') ? 'is-invalid' : '' }}" name="meta[nrc_township]" id="nrc_township">
                                <option value="">--</option>
                            </select>
                        </div>

                        <div class="col-3">
                            <select class="form-control {{ $errors->has('meta.nrc_type') ? 'is-invalid' : '' }}" name="meta[nrc_type]" id="nrc_type">
                                <option value="">--</option>
                            </select>
                        </div>
                        <div class="col-3">
                            <input class="form-control {{ $errors->has('meta.nrc_no') ? 'is-invalid' : '' }}"
                                   type="text"
                                   name="meta[nrc_no]" id="nrc_no" value="{{ old('meta.nrc_no', '') }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('meta.dob') ? 'is-invalid' : '' }}"
                           type="text"
                           name="meta[dob]"
                           id="dob"
                           value="{{ old('meta.dob', '') }}"                        
                ...
                <div class="form-group">
                    <label for="address" class="form-label required"> {{ trans('global.address') }} </label>
                    <textarea name="meta[address]" id="address" class="form-control {{ $errors->has('meta.address') ? 'is-invalid' : '' }}">{{ old('meta.address') }}</textarea>
                    @error('meta.address')
                    <div class="invalid-feedback">
            ...
            <div
                class="form-group licence-no"
                style="display:{{ old('role_id') == config('constants.role_id.professional') ? 'block' : 'none'}};"
            ...
            >
                ...
                <input class="form-control {{ $errors->has('meta.licence_no') ? 'is-invalid' : '' }}" type="text" name="meta[licence_no]" id="licence_no" value="{{ old('meta.licence_no', '') }}">
            ...
            <div
                class="form-group speciality-group"
                style="display:{{ old('role_id') == config('constants.role_id.professional') ? 'block' : 'none'}};"
                data-visible="{{ config('constants.role_id.professional') }}"
            >
            ...
            <div
                class="form-group business-group"
                style="display:{{ old('role_id') == config('constants.role_id.clinic') ||
                    old('role_id') == config('constants.role_id.business') ? 'block' : 'none'}};"
            ...
            >
                ...
                <select class="form-control select2 {{ $errors->has('businesses') ? 'is-invalid' : '' }}" name="businesses[]" id="businesses" multiple>
                    @foreach($businesses as $id => $business)
                        <option value="{{ $id }}" {{ in_array($id, old('businesses', [])) ? 'selected' : '' }}>{{ $business }}</option>
                    @endforeach
                </select>
...
<script>
    $(function () {
        ...
        let oldData = !Array.isArray(oldJson) ? oldJson.meta : [];
